...
primary_completion_Africa <- paste0(
  sprintf("%.1f",mean(LE_Africa$ed_attain_primary_rate, na.rm = TRUE)), ' (', sprintf("%.1f", sd(LE_Africa$ed_attain_primary_rate, na.rm = TRUE)), ")")
primary_completion_Asia <- paste0(
  sprintf("%.1f",mean(LE_Asia$ed_attain_primary_rate, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Asia$ed_attain_primary_rate, na.rm = TRUE)), ")")
primary_completion_Oceania <- paste0(
  sprintf("%.1f",mean(LE_Oceania$ed_attain_primary_rate, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Oceania$ed_attain_primary_rate, na.rm = TRUE)), ")")
primary_completion_Europe <- paste0(
  sprintf("%.1f",mean(LE_Europe$ed_attain_primary_rate, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Europe$ed_attain_primary_rate, na.rm = TRUE)), ")")
primary_completion_NA <- paste0(
  sprintf("%.1f",mean(LE_NA$ed_attain_primary_rate, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_NA$ed_attain_primary_rate, na.rm = TRUE)), ")")
primary_completion_SA <- paste0(
  sprintf("%.1f",mean(LE_SA$ed_attain_primary_rate, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_SA$ed_attain_primary_rate, na.rm = TRUE)), ")")
primary_completion_All <- paste0(
  sprintf("%.1f",mean(LE_data$ed_attain_primary_rate, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_data$ed_attain_primary_rate, na.rm = TRUE)), ")")
...
primary_completion_pmissing <- paste0(
  sprintf("%.1f",mean(is.na(LE_data$ed_attain_primary_rate))*100), '%')
...
p3 <- ggplot(data = LE_data, aes(x = health_expend_pc, 
  ...
  scale_x_log10() + 
  theme_bw()+
  ylab("Population (in 10,000)") +
  xlab("Healthcare Expenditure Per Capita ($)") +
  theme(plot.margin = unit(c(1,0.5,0.5,0.5), "cm"))

top <- cowplot::plot_grid(p1, p2, labels = c("A. Histogram of Life Expectancy", "B. Boxplot of Life Expectancy by GDP Quantile"),
                          hjust = -0.1)
...
imputations <- mice(LE_data2, seed = 2310, method = "pmm", m = 15)
...
corrplot(as.matrix(cor_df),tl.cex = 0.5)

# VIF. Find mean over imputations
vif_output <- list()
for (i in 1:15){
...
We see a number of variables that have high VIFs and correlations with similar values with absolute values > 0.8. Based on these observations, we decide to remove national net income growth because of its high correlation with national net income growth per capita (r = 0.979), birth rate because of its high positive correlation with population growth rate (r = 0.76), infant mortality rate (r = 0.86) and large negative correlation with safe drinking water rate (r = - 0.85). Removing these helps us address multicollinearity which makes our coefficients and their standard errors more appropriate. We also decide to remove electricity access because of its high VIF and large correlations with infant mortality rate (r = 0.81), primary completion rate (r = 0.71), and safe drinking water rate (r = 0.72).
...
#Define the analysis you want to perform at Stage 2
feature.selection1 <- expression(null.model1 <- lm(life_expectancy ~ 1), model2 <- step(null.model1, scope = ~  income_growth_pc + children_out_of_school + infant_mortality_rate + primary_completion_rate + real_interest_rate + pop_growth_perc_annual + pop_dens + pop_total + health_expend_pc + health_expend_perc_gdp + unemployment_rate + gdp_growth + gdp_pc + adults_HIV + safe_drinking_water + ed_duration))
...
fit <- with(imputations, lm(life_expectancy ~ 
         adults_HIV + infant_mortality_rate + safe_drinking_water + 
         health_expend_pc + pop_dens))
...
We see that in this final model, all five features are statistically significantly associated with life expectancy at the 5% level. The coefficients, errors, and p-values are displayed in the table above. After controlling for the other features, the a one unit increase in infant mortality rate is associated with a -0.26 year increase (0.26 year decrease) in life expectancy. Altogether, our model provides evidence that the number of adults newly infected with HIV and the infant mortality rate (possible proxy variables for weaker health system) are associated with lower life expectancies on average, while the increased percents of populations using safe drinking water, health expenditures per capita, and population densities are associated with higher life expectancies on average. 

To reassure ourselves that this is an appropriate final model, we run some model diagnostic tests (with some help from Solomon Kurz blog post).  
...
for (i in 1:15){
  vif_output[[i]] <- faraway::vif(dplyr::select(
    complete(imputations, i), adults_HIV, infant_mortality_rate,
     safe_drinking_water, health_expend_pc, pop_dens))
}
vif_output_means2 <- c()
for (i in 1:length(vif_output[[1]])){
  val_vec <- c()
...
First, we check the variance inflation factors of the features selected for our final model. They are all below four, which is a good sign that multicollinearity is not too much of a problem.
...
predict_df <- cbind(
  ...
  predict(fit$analyses[[11]], newdata = complete(imputations, 11)),
  predict(fit$analyses[[12]], newdata = complete(imputations, 12)),
  predict(fit$analyses[[13]], newdata = complete(imputations, 13)),
  predict(fit$analyses[[14]], newdata = complete(imputations, 14)),
  predict(fit$analyses[[15]], newdata = complete(imputations, 15))
)
...
observed_df <- cbind(
  ...
  dplyr::select(complete(imputations, 11), life_expectancy),
  dplyr::select(complete(imputations, 12), life_expectancy),
  dplyr::select(complete(imputations, 13), life_expectancy),
  dplyr::select(complete(imputations, 14), life_expectancy),
  dplyr::select(complete(imputations, 15), life_expectancy)
)

residual_df <- predict_df - observed_df
names(residual_df) <- c(1:15)
...
le.aov <- aov(life_expectancy ~ health_expend_perc_gdp + Continent,
               data = anova_data)
summary(le.aov)
...
p1 <- ggplot(anova_data, aes(x = health_expend_perc_gdp, 
                             y = life_expectancy, 
                             color = Continent)) +
  theme_bw()+
  geom_point()+
