...
primary_completion_Africa <- paste0(
  sprintf("%.1f",mean(LE_Africa$SE.PRM.CMPT.ZS, na.rm = TRUE)), ' (', sprintf("%.1f", sd(LE_Africa$SE.PRM.CMPT.ZS, na.rm = TRUE)), ")")
primary_completion_Asia <- paste0(
  sprintf("%.1f",mean(LE_Asia$SE.PRM.CMPT.ZS, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Asia$SE.PRM.CMPT.ZS, na.rm = TRUE)), ")")
primary_completion_Oceania <- paste0(
  sprintf("%.1f",mean(LE_Oceania$SE.PRM.CMPT.ZS, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Oceania$SE.PRM.CMPT.ZS, na.rm = TRUE)), ")")
primary_completion_Europe <- paste0(
  sprintf("%.1f",mean(LE_Europe$SE.PRM.CMPT.ZS, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Europe$SE.PRM.CMPT.ZS, na.rm = TRUE)), ")")
primary_completion_NA <- paste0(
  sprintf("%.1f",mean(LE_NA$SE.PRM.CMPT.ZS, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_NA$SE.PRM.CMPT.ZS, na.rm = TRUE)), ")")
primary_completion_SA <- paste0(
  sprintf("%.1f",mean(LE_SA$SE.PRM.CMPT.ZS, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_SA$SE.PRM.CMPT.ZS, na.rm = TRUE)), ")")
primary_completion_All <- paste0(
  sprintf("%.1f",mean(LE_data$SE.PRM.CMPT.ZS, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_data$SE.PRM.CMPT.ZS, na.rm = TRUE)), ")")
...
health_expenditure_Africa <- paste0(
  sprintf("%.1f",mean(LE_Africa$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ' (', sprintf("%.1f", sd(LE_Africa$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ")")
health_expenditure_Asia <- paste0(
  sprintf("%.1f",mean(LE_Asia$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Asia$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ")")
health_expenditure_Oceania <- paste0(
  sprintf("%.1f",mean(LE_Oceania$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Oceania$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ")")
health_expenditure_Europe <- paste0(
  sprintf("%.1f",mean(LE_Europe$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_Europe$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ")")
health_expenditure_NA <- paste0(
  sprintf("%.1f",mean(LE_NA$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_NA$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ")")
health_expenditure_SA <- paste0(
  sprintf("%.1f",mean(LE_SA$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_SA$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ")")
health_expenditure_All <- paste0(
  sprintf("%.1f",mean(LE_data$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ' (', sprintf("%.1f",sd(LE_data$SH.XPD.CHEX.PC.CD, na.rm = TRUE)), ")")
...
life_expectancy_pmissing <- paste0(
  sprintf("%.1f",mean(is.na(LE_data$SP.DYN.LE00.IN))*100), '%')

primary_completion_pmissing <- paste0(
  sprintf("%.1f",mean(is.na(LE_data$SE.PRM.CMPT.ZS))*100), '%')

pop_density_pmissing <- paste0(
  sprintf("%.1f",mean(is.na(LE_data$EN.POP.DNST))*100), '%')

health_expenditure_pmissing <- paste0(
  sprintf("%.1f",mean(is.na(LE_data$SH.XPD.CHEX.PC.CD))*100), '%')

GDP_pmissing <- paste0(
  sprintf("%.1f",mean(is.na(LE_data$NY.GDP.MKTP.KD.ZG))*100), '%')
...
```{r}
LE_data_test <- LE_data2
x <- c()

for (i in 1:ncol(LE_data_test))
 {
   x[i] = class(LE_data_test[,i])
 }
table(x)

LE_data_test$SP.POP.TOTL <- as.numeric(LE_data_test$SP.POP.TOTL)
LE_data_test$SE.COM.DURS <- as.numeric(LE_data_test$SE.COM.DURS)

noise <- rnorm(nrow(LE_data2), 0 , 500)

LE_data2$SP.POP.TOTL <- LE_data2$SP.POP.TOTL/100000


imputations <- mice(LE_data2, seed = 2310, method = "pmm")
print(imputations)
View(complete(imputations))
...
model1 <- lm(SP.DYN.LE00.IN ~ ., data=complete(imputations))
summary(model1)

corr <- cor(complete(imputations)[,-1])
corr
...
```{r}
library(car)
anova_LE <- aov(SP.DYN.LE00.IN ~ as.factor(Continent), data=complete(imputations))
summary(anova_LE)
