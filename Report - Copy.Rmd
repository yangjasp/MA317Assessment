...
# Mean and standard deviation of the 5 variables we're studying # In order for us to visualize the data in a "easier" way, we're going summarize it in a table
...
# Table titles
top <- cowplot::plot_grid(p1, p2, labels = c("A. Histogram of Life Expectancy", "B. Life Expectancy by GDP"), hjust = -0.1)
bottom <- cowplot::plot_grid(p3, labels = c("C. Life Expectancy by Population and Healthcare Spending"), hjust = -0.1)
...
#???????????????????????????????????????????????????????????????????????????????????????????? (we need to either use it or delete it)
...
# We are using the mice package, which multiple imputation
...
imputations <- mice(LE_data2, seed = 231, predictorMatrix = pred, method = "pmm", m = 10)

imputations$imp #do we really need to print this ???????????????????????????????????????????????????????????????????????????????????????????
#stripplot(imputations, pch = 20, cex = 1.2) ?????????????????????????????????????????????????????????? (we need to either use it or delete it)
#xyplot(imputations, bmi ~ chl | .imp, pch = 20, cex = 1.4)???????????????????????????????????????????? (we need to either use it or delete it)

#Defining the analysis we're performing at Stage 2
feature.selection1 <- expression(null.model1 <- lm(life_expectancy ~ 1),
                                 model2 <- step(null.model1, scope = ~ Continent + electricity_access +
                                                  income_growth + income_growth_pc + children_out_of_school +
                                                  infant_mortality_rate + primary_completion_rate +
                                                  real_interest_rate + pop_growth_perc_annual + pop_dens +
                                                  pop_total + health_expend_pc + health_expend_perc_gdp +
                                                  unemployment_rate + gdp_growth + gdp_pc + birth_rate +
                                                  adults_HIV + safe_drinking_water + ed_duration ))
...
feature.frequency #???????????????????????????????????????????????????????????????????????? do we need this?????
...
# i did not understand which missing values. just the LE? The ones we saw before? The imputations??????????????????????

# Countries missing life expectancy data
countries_missing_LE <- dplyr::filter(LE_data, is.na(life_expectancy))$Country.Name

vars <- c("life_expectancy", "adults_HIV", "health_expend_pc", "infant_mortality_rate") #why this variables????????????????????????????
...
p2 <- ggplot(imps, aes(x = imp, 
                       y = safe_drinking_water, #isn't it supposed to be adults_HIV ?????????????????????????????????????????????????????????
                       color = Type)) +
...
# Studying collinearity presence with miceadds package
...
#corrplot.mixed(as.matrix(cor_df), lower.col = "black", number.cex = 0.7, tl.cex = 0.5) #??????????????? delete??????????? perhaps?
corrplot(as.matrix(cor_df),tl.cex = 0.8, tl.col="black")


# I dont think it is really cleat what the vif function is doing ???????????????????????????????????????????????????????????????????
# WHICH ONE ARE WE KEEPING????? I THINK THE ONE BWLOW IT'S BETTER............ ????????????????????????????????
...
drop_vars <- c("Continent","life_expectancy","income_growth","birth_rate", "electricity_access", "health_expend_pc") #why this??????????????
...
#THERE'S AN ERROR SOMEWHERE CAUSE THE TABLE IS NOT PRINTING THE VIF AFTER DROPPING VARS BUT I CANT FIND IT ????????????????????
# ARE WE KEEPING THIS THO??????????????????????????????????????????????????????????????????????????????????????????????????????????
...
# WE'VE DONE THIS, DELETE???????????????????????????????????????????????????????????????????????????????????????????
stripplot(imputations, pch = 20, cex = 1.2) #??????????????? delete??????????????????? maybe?????????????????????????????????????
#xyplot(imputations, bmi ~ chl | .imp, pch = 20, cex = 1.4) #??????????????? delete??????????????????? maybe?????????????????????????????????????
...
# Pooling from above model (would be better to take that down till here)
...
# Checking the normality asumptions of the model
predict_df <- cbind(
...
residual_df <- predict_df - observed_df # We can observe residual_df has 15 columns (one for each imp) and 217  rows (one per obs).
...
p1 <- ggplot(anova_data, aes(x = log(gdp_pc), y = life_expectancy, color = Continent)) +
  theme_bw() + geom_point(alpha = 0.5, size = 0.8) + xlab("log(GDP per capita in thousands)")+
...
# Shapiro test
shapiro.test(anova_data$residuals) #It said shapiro.test(anova_data$residuals1), so I took the 1 out, im not sure this is right tho ?????????????????????????
```
...
# ARE WE SUPPOSED TO WRITE THE NULL HYP OR WHAT?????????????????????????????????????????????????????????im done with the question marks haha
